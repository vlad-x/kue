...
Job.prototype.removeState = function (fn) {
    ...
    if( state && state != '' ) {
        client.multi()
            .zrem(client.getKey('jobs:' + state), paddedID)
            .zrem(client.getKey('jobs:' + this.type + ':' + state), paddedID)
        .exec( fn );
    } else {
        fn && fn();
    }
...
/**
>>>>>>> Uses zero-padded id in redis to maintain sorting.
 * Set state to `state`.
 *
 * @param {String} state
 * @param fn
...
Job.prototype.update = function (fn) {
    ...
    var id = this.id
    var paddedID = zeropad(id)
    this.client.zadd(this.client.getKey('jobs'), this._priority, paddedID);
