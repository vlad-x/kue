...
exports.removeBadJob = function (id, hash) {
    var client = redis.client();
    var paddedID = zeropad(id)
    if( hash && hash.state ) {
        client.zrem(client.getKey('jobs:' + hash.state), paddedID);
    }
    client.multi()
        .del(client.getKey('job:' + id + ':log'))
        .del(client.getKey('job:' + id))
        .zrem(client.getKey('jobs'), paddedID)
    .exec();
...
Job.prototype.remove = function (fn) {
    ...
    var paddedID = zeropad(this.id);
    client.multi()
        .zrem(client.getKey('jobs:' + this.state()), paddedID)
        .zrem(client.getKey('jobs:' + this.type + ':' + this.state()), paddedID)
        .zrem(client.getKey('jobs'), paddedID)
        .del(client.getKey('job:' + paddedID + ':log'))
        .del(client.getKey('job:' + paddedID))
        .exec( function( err ){
//            events.remove(this);
        if( !exports.disableSearch ){
            getSearch().remove(paddedID, fn);
        } else {
            fn && fn(err);
        }
    }.bind(this));
...
<<<<<<< HEAD
    ...
    if( oldState && oldState != '' && oldState != state ) {
        multi
            .zrem(client.getKey('jobs:' + oldState), paddedID)
            .zrem(client.getKey('jobs:' + this.type + ':' + oldState), paddedID);
    }
=======
    if( oldState && oldState != '' && oldState != state ) {
        multi
...
>>>>>>> more atomic job state changes, #411
    multi
        .hset(client.getKey('job:' + paddedID), 'state', state)
        .zadd(client.getKey('jobs:' + state), this._priority, paddedID)
        .zadd(client.getKey('jobs:' + this.type + ':' + state), this._priority, paddedID);
...
>>>>>>> more atomic job state changes, #411
    ...
    multi.exec(function (err, replies) {
        if(!err) {
            ...
            this.set('updated_at', Date.now());
<<<<<<< HEAD
            (this._state === 'inactive') ? events.emit(paddedID, 'enqueue', this.type) : noop();
=======
>>>>>>> more atomic job state changes, #411
        }
        return fn( err );
